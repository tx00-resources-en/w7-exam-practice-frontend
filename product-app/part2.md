...
In Part 1 you built a full CRUD front-end for products using the **unprotected** `backend/` API.  
...
| Folder | Purpose |
|---|---|
| `product-app/backend-auth/` | Express API with user signup/login endpoints. No route protection. |
| `product-app/backend-protect/` | Express API with user signup/login **and** `requireAuth` middleware on POST/PUT/DELETE product routes. |
| `product-app/frontend/step5/` | Your **starting point** for Iteration 6. Copy this folder and work inside the copy. |
| `product-app/frontend/step6/` | Sample solution for Iteration 6. Only look at it **after** you have tried on your own. |
| `product-app/frontend/step7/` | Sample solution for Iteration 7. Only look at it **after** you have tried on your own. |
...
| 7 | Route protection & token headers | `backend-protect/` | `App.jsx`, `Navbar.jsx`, `Signup.jsx`, `Login.jsx`, `ProductPage.jsx`, `AddProductPage.jsx`, `EditProductPage.jsx` |
...
This backend is identical to `backend/` but adds **user routes**. All product routes remain **unprotected**.
...
### `backend-protect/` — Protected Product Routes (Iteration 7)

This backend is identical to `backend-auth/` except that **creating, updating, and deleting** products requires a valid JWT in the `Authorization` header.
...
> **What is different in `backend-protect/`?** The product router now uses a `requireAuth` middleware on POST, PUT, and DELETE. Any request to these endpoints **must** include an `Authorization: Bearer <token>` header, or it will be rejected with a 401 error.
